...
contract NFTAuctionV1Test is Test {
    ...
    Harness_NFTAuction public harness = new Harness_NFTAuction();
    ...
    uint256 public constant SEPOLIA_TOKENID_1 = 159;
    uint256 public constant SEPOLIA_TOKENID_2 = 189;
    ...
    function setUp() public {
        ...
        if (isForkMode) {
            ...
            sepoliaNFT1Owner = ERC721(SEPOLIA_NFT_ADDR).ownerOf(SEPOLIA_TOKENID_1);
    ...
    function test_batchAddTokenCfg_Success() public {
        ...
        emit NFTAuctionV1.AddTokenCfg(auctionSysOwner, token1, tokenAddr1, feedAddr1, block.timestamp);
        vm.expectEmit(true, true, true, true, auctionSysProxyAddr, 1);
        emit NFTAuctionV1.AddTokenCfg(auctionSysOwner, token2, tokenAddr2, feedAddr2, block.timestamp);
    ...
    function test_batchAddTokenCfg_InvalidTokenAddr1() public {
        ...
        address testFeedAddr = 0x1a81afB8146aeFfCFc5E50e8479e826E7D55b910;
        _hackSetEnabledTokens(0, false);

        NFTAuctionV1.TokenInitConfig[] memory tokenInitList = new NFTAuctionV1.TokenInitConfig[](1);
        tokenInitList[0] = NFTAuctionV1.TokenInitConfig({token: 0, tokenAddr: testTokenAddr, feedAddr: testFeedAddr});
    ...
    function test_batchAddTokenCfg_InvalidTokenAddr2() public {
        // 为测试创造条件，先修改storage里的值
        _hackSetEnabledTokenAddrs(address(0), false);
    ...
    function test_addTokenCfg_Success() public {
        ...
        vm.prank(auctionSysOwner);
        vm.expectEmit(true, true, true, true, auctionSysProxyAddr, 1);
        emit NFTAuctionV1.AddTokenCfg(auctionSysOwner, token, tokenAddr, feedAddr, block.timestamp);
    ...
    function test_updCfgTokenAddr_Success() public {
        ...
        vm.prank(auctionSysOwner);
        vm.expectEmit(true, true, true, true, auctionSysProxyAddr, 1);
        emit NFTAuctionV1.UpdCfgTokenAddr(auctionSysOwner, token, beforeTokenAddr, tokenAddr, block.timestamp);
    ...
    // updCfgTokenAddr: token==0 revert
    function test_updCfgTokenAddr_RevertIf_InvalidAllowedToken() public {
        vm.prank(auctionSysOwner);
        vm.expectRevert(abi.encodeWithSignature("InvalidAllowedToken(uint256)", 0));
        NFTAuctionV1(auctionSysProxyAddr).updCfgTokenAddr(0, address(0));
    }
    ...
    function test_updCfgFeedAddr_Success() public {
        ...
        vm.prank(auctionSysOwner);
        vm.expectEmit(true, true, true, true, auctionSysProxyAddr, 1);
        emit NFTAuctionV1.UpdCfgFeedAddr(auctionSysOwner, token, beforeFeedAddr, feedAddr, block.timestamp);
    ...
    function test_delTokenCfg_Success() public {
        ...
        vm.prank(auctionSysOwner);
        vm.expectEmit(true, true, true, true, auctionSysProxyAddr, 1);
        emit NFTAuctionV1.DelTokenCfg(auctionSysOwner, token, tokenAddr, feedAddr, block.timestamp);
    ...
    // delTokenCfg: token==0 revert
    function test_delTokenCfg_RevertIf_InvalidAllowedToken() public {
        vm.prank(auctionSysOwner);
        vm.expectRevert(abi.encodeWithSignature("InvalidAllowedToken(uint256)", 0));
        NFTAuctionV1(auctionSysProxyAddr).delTokenCfg(0);
    }
    ...
    function test_createAuction_Success1() public {
        ...
        allowedTokens[1] = 1;
        allowedTokens[2] = 2;
        NFTAuctionV1.TokenInitConfig[] memory allowedTokensList = _getExpectAllowedTokens(allowedTokens.length);
        address nftContract;
        address caller;
    ...
    function test_createAuction_Success2() public {
        uint256[] memory allowedTokens1 = new uint256[](3);
        allowedTokens1[0] = 0;
        allowedTokens1[1] = 1;
        allowedTokens1[2] = 2;
        uint256[] memory allowedTokens2 = new uint256[](1);
        allowedTokens2[0] = 1;
        uint256[] memory allowedTokens3 = new uint256[](2);
        allowedTokens3[0] = 1;
        allowedTokens3[1] = 0;
        NFTAuctionV1.TokenInitConfig[] memory allowedTokensList = _getExpectAllowedTokens(3);
        address nftContract;
        address seller;
        uint256 tokenId1;
        uint256 tokenId2;
        ...
        assertEq(NFTAuctionV1(auctionSysProxyAddr).getAuctionCount(), 1);
        assertEq(NFTAuctionV1(auctionSysProxyAddr).getNtfToken2AuctionId(nftContract, tokenId1), auctionId1);
        assertEq(ERC721(nftContract).ownerOf(tokenId1), auctionSysProxyAddr);
        ...
        assertEq(NFTAuctionV1(auctionSysProxyAddr).getNtfToken2AuctionId(nftContract, tokenId2), auctionId2);
        ...
        assertEq(NFTAuctionV1(auctionSysProxyAddr).getAuctionCount(), 1);
        assertEq(NFTAuctionV1(auctionSysProxyAddr).getNtfToken2AuctionId(nftContract, tokenId1), auctionId1);
        assertEq(ERC721(nftContract).ownerOf(tokenId1), seller);

        // 4. 对拍卖 1 再次创建拍卖
        NFTAuctionV1.CreateAuctionParams memory params3 = NFTAuctionV1.CreateAuctionParams({
            nftContract: nftContract,
            tokenId: tokenId1,
            startPrice: 1,
            startTime: currTs + 1 minutes,
        ...
        uint256 auctionId12 = NFTAuctionV1(auctionSysProxyAddr).createAuction(params3);
        assertEq(NFTAuctionV1(auctionSysProxyAddr).getNtfToken2AuctionId(nftContract, tokenId1), auctionId12);
        assertEq(ERC721(nftContract).ownerOf(tokenId1), auctionSysProxyAddr);
        NFTAuctionV1.AuctionInfo memory info2 = NFTAuctionV1(auctionSysProxyAddr).getAuctionInfo(auctionId12);
        assertEq(info2.allowedTokens, allowedTokens3);
    ...
    // createAuction测试场景：待创建拍卖的allowedTokens长度超标 revert
    function test_createAuction_RevertIf_AllowedTokenSizeOver() public {
        uint256[] memory allowedTokens = new uint256[](6);
        allowedTokens[0] = 0;
        allowedTokens[1] = 1;
        allowedTokens[2] = 2;
        allowedTokens[3] = 3;
    ...
    function test_createAuction_RevertIf_NftCreated() public {
        uint256[] memory allowedTokens;
        address nftContract;
        uint256 tokenId = APPLENFT_TOKENID_1;
        address seller;
        ...
        // 再次对指定NFT创建拍卖
        vm.expectRevert(abi.encodeWithSignature("NotNftOwner(address)", seller));
        NFTAuctionV1(auctionSysProxyAddr)
            .createAuction(
                NFTAuctionV1.CreateAuctionParams({
                nftContract: nftContract,
    ...
    // createAuction测试场景：创建拍卖 -> 取消拍卖 -> isCreated=true -> revert
    function test_createAuction_AuctionAlreadyExists() public {
        uint256[] memory allowedTokens;
        address nftContract;
        uint256 tokenId = APPLENFT_TOKENID_1;
        address seller;
        ...
        // 第一次对指定NFT创建拍卖
        uint256 auctionId1 = NFTAuctionV1(auctionSysProxyAddr)
            .createAuction(
                NFTAuctionV1.CreateAuctionParams({
                nftContract: nftContract,
                tokenId: tokenId,
    ...
    function test_cancelAuction_Success() public {
        ...
        assertEq(ERC721(nftContract).ownerOf(APPLENFT_TOKENID_1), caller);
    }
    ...
    function test_endAuction_Success1() public {
        ...
        assertEq(ERC721(nftContract).ownerOf(tokenId), auctionSysProxyAddr);
    ...
    //     vm.startPrank(0x8888888888888888888888888888888888888888);
    //     v1Handler.create(
    //         84195578566463107549075102294111567,
    //         160526445051572190132445332369302091309272155,
    //         48263816075543302375011714525153460870515217540,
    //         250901436463159699122693104985627910846089377235596971437571219909993467810,
    //         0
    //     );
    //     vm.stopPrank();

    //     vm.startPrank(0xa39434a63A52E9B1C0C696241E2288EAE7071207);
    //     v1Handler.bid(
    //         311172477367625877144089287709280777257298895940,
    //         19327981731530995928116050188309402675817999553156945,
    //         458090271506880574966476951282715386613846877435464928633349,
    //         379716545717840151744093730979270624441233906863970108
    //     );
    //     vm.stopPrank();

    //     vm.startPrank(0x4444444444444444444444444444444444444444);
    //     v1Handler.create(
    //         15580532,
    //         3,
    //         322788324789663718188158436794152977338259182784,
    //         1026086634991000708852059670400686316220754309516690229007909162818,
    //         24683971725759765515188806651562041377211587181785616464205475168
    //     );
    //     vm.stopPrank();

    //     console.log(auctionSysProxyAddr.balance);
    //     assertGe(
    //         auctionSysProxyAddr.balance,
    //         (v1Handler.ghost_bidSum() - v1Handler.ghost_refundSum() - v1Handler.ghost_endValueSum())
    //     );
    ...
    // 设置storage中enabledTokenAddrs的值
    function _hackSetEnabledTokenAddrs(address tokenAddr, bool value) internal {
        // mapping 基础slot = 根槽 + 相对偏移8
        bytes32 mapBase = bytes32(uint256(AUCTION_STORAGE_LOCATION) + 8);
        // 第一层key：token
        bytes32 finalSlot = keccak256(abi.encode(tokenAddr, mapBase));
        // bool只看最低bit
        bytes32 storeVal = value ? bytes32(uint256(1)) : bytes32(uint256(0));
        vm.store(auctionSysProxyAddr, finalSlot, storeVal);
    }

    // 设置storage中enabledTokens的值
    function _hackSetEnabledTokens(uint256 token, bool value) internal {
        // mapping 基础slot = 根槽 + 相对偏移6
        bytes32 mapBase = bytes32(uint256(AUCTION_STORAGE_LOCATION) + 6);
        // 第一层key：token
        bytes32 finalSlot = keccak256(abi.encode(token, mapBase));
        // bool只看最低bit
        bytes32 storeVal = value ? bytes32(uint256(1)) : bytes32(uint256(0));
    ...
    // // 扫描AuctionInfo内部0~19相对slot，定位bool所在slot
    // function _findBoolPackSlot(uint256 auctionId) internal view {
    //     bytes32 mapBase = bytes32(uint256(AUCTION_STORAGE_LOCATION) + 2);
    //     bytes32 auctionInfoRootSlot = keccak256(abi.encode(auctionId, mapBase));

    //     // 扫描前20个相对slot，足够覆盖你的AuctionInfo全部字段
    //     for(uint256 offset = 0; offset < 14; offset++){
    //         bytes32 slot = bytes32(uint256(auctionInfoRootSlot) + offset);
    //         bytes32 raw = vm.load(auctionSysProxyAddr, slot);
    //         uint256 val = uint256(raw);
    //         console.log("offset=", offset);
    //         console.logBytes32(raw);
    //         // // isCreated = true 时最低字节不为0
    //         // if(offset > 9 && (val & 0xff) != 0){
    //         //     return offset;
    //         // }
    //     }
    //     //revert("bool slot not found");
    // }

    // 修改storage中AuctionInfo.isCreated值
    function _hackSetIsCreated(uint256 auctionId, bool flag) internal {
        // auctions mapping 在AuctionStorage内偏移 = 2
        bytes32 mapBase = bytes32(uint256(AUCTION_STORAGE_LOCATION) + 2);
        // mapping(key, slot) key = auctionId
        bytes32 auctionInfoRootSlot = keccak256(abi.encode(auctionId, mapBase));

        // ========== 风险点：这个10是人工数出来的相对slot ==========
        // 只要AuctionInfo增删/重排字段，这个数字必须重新核对！
        bytes32 boolPackSlot = bytes32(uint256(auctionInfoRootSlot) + 9);

        bytes32 rawBytes = vm.load(auctionSysProxyAddr, boolPackSlot);
        uint256 slotVal = uint256(rawBytes);
        //console.logBytes32(rawBytes);

        uint256 bitPos = 160; // isCreated 在 bit160
        uint256 mask = uint256(1) << bitPos;

        if (flag) {
            // 设置第bitPos位为true
            slotVal = slotVal | mask;
            //console.logBytes32(bytes32(slotVal));
        } else {
            // 设置第bitPos位为false
            slotVal = slotVal & ~mask;
        }
